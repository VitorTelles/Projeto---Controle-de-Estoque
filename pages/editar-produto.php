<?php
	$id = (int)$_GET['id'];
	$sql = MySql::conectar()->prepare("SELECT * FROM `tb_produtos` WHERE id = ?");
	...
	$pegaImagens = MySql::conectar()->prepare("SELECT * FROM `tb_produtos.imagens` WHERE produto_id = $id");
	$pegaImagens->execute();
...
<div class="box-content">
	...
	if(isset($_GET['deletarImagem'])){
		...
		@unlink(BASE_DIR.'/imagens/'.$idImagem);
		MySql::conectar()->exec("DELETE FROM `tb_produtos.imagens` WHERE imagem = '$idImagem'");
	...
	?>
	<form method="post" action="<?php echo INCLUDE_PATH?>editar-produto?id=<?php echo $id; ?>" enctype="multipart/form-data">
	<?php
		if(isset($_POST['acao'])){
			$nome = $_POST['nome'];
			...
			if($sucesso){
				if($_FILES['imagem']['name'][0] != ''){
					...
					foreach ($imagens as $key => $value) {
						MySql::conectar()->exec("INSERT INTO `tb_produtos.imagens` VALUES (null,$id,'$value')");
					}
				}

				$sql = MySql::conectar()->prepare("UPDATE `tb_produtos` SET nome = ?,descricao = ?, largura = ?,
				altura = ?, comprimento = ?, peso = ?, preco = ?,quantidade = ? WHERE id = $id");
	...
	<div class="form-group">
        ...
        <div class="form-group">
			...
			<input type="number" name="preco" min="0" max="900" step="1" value="<?php echo $infoProduto['preco']; ?>">
		</div><!--form-group-->
    ...
    <div class="boxes">
            ...
            <div class="box-single-wraper">
                ...
                <div style="width: 100%;float: left;" class="box-imgs">
                    <img class="img-square" src="<?php echo INCLUDE_PATH?>imagens/<?php echo $value['imagem'] ?>" />
                </div><!--box-imgs-->
                <div class="clear"></div>
                <div style="text-align: center;" class="group-btn">
                    <a class="btn delete" href="<?php echo INCLUDE_PATH?>editar-produto?id=<?php echo $id; ?>&deletarImagem=<?php echo $value['imagem'] ?>"><i class="fa fa-times"></i> Excluir</a>
                </div><!--group-btn-->
